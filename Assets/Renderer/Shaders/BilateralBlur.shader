Shader "Postprocessing/BilateralBlur"
{
    ...
    SubShader
    {
        ...
        half4 frag(Varyings input) : SV_Target
        {
            ...
            half3 col = color.rgb;
            float depthBlurFactor = GetDepthBlendFactor(input.uv, _DepthFocus, _DepthSmoothness, _DepthBlendPower);
            ...
            for (int y = -COUNT; y <= COUNT; y++)
            {
                ...
                {
                    half3 _col = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv + float2(x, y) * _Spread * depthBlurFactor).rgb;
